...
export class IncomeController {
  ...
  @ApiResponse({ status: 200, description: 'Returns income records with pagination.' })
  ...
  @ApiQuery({ name: 'paymentMethodId', required: false, description: 'Filter by payment method ID' })
  @ApiQuery({ name: 'searchTerm', required: false, description: 'Search in description' })
  @ApiQuery({ name: 'minAmount', required: false, description: 'Minimum amount in cents' })
  @ApiQuery({ name: 'maxAmount', required: false, description: 'Maximum amount in cents' })
  ...
  @ApiResponse({ status: 200, description: 'Income record voided successfully.' })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  @ApiResponse({ status: 404, description: 'Income record not found.' })
  @ApiQuery({ name: 'id', required: true, description: 'Income record ID to void' })
